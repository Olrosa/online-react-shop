import { useState, useEffect } from 'react';
import slide1 from '../../resources/slides/slide1.webp';
import slide2 from '../../resources/slides/slide2.webp';
import slide3 from '../../resources/slides/slide3.webp';

import './banner.scss';
import '../../style/slider.scss';

const Banner = ({slideNum, autoplaySlider}) => {
    const slides = [
        { img: slide1, num: 0}, { img: slide2, num: 1}, { img: slide3, num: 2}
    ];

    const [slide, setSlide] = useState(slides[slideNum]);
    const [autoplay, setAutoplay] = useState(autoplaySlider);

    const nextSlide = () => {
        const currentSlideNum = slide.num;
        slides[currentSlideNum + 1] ? setSlide(slides[currentSlideNum + 1]) : setSlide(slides[0]);
        setAutoplay(false);
    }

    const prevSlide = () => {
        const currentSlideNum = slide.num;
        const slidesLength = slides.length - 1;
        slides[currentSlideNum - 1] ? setSlide(slides[currentSlideNum - 1]) : setSlide(slides[slidesLength]);
        setAutoplay(false);
    }

    useEffect(() => {
        if (autoplay) {
            const playSlider = () => {
                const currentSlideNum = slide.num;
                slides[currentSlideNum + 1] ? setSlide(slides[currentSlideNum + 1]) : setSlide(slides[0]);
            }
            const interval = setInterval(playSlider, 5000);
            return () => clearInterval(interval);
        }
    }, [autoplay, slide]);


    return (
        <div className='slider'>
            <div className='wrapper'>
                <div className='slider-offer'>
                    <div className='slider-slide'>
                        <img className='slider-img' src={slide.img} />
                    </div>

                    <svg onClick={nextSlide} className='slider-arrow slider-arrow-next' xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"  viewBox="0 0 16 16">
                        <path fillRule="evenodd" d="M4 8a.5.5 0 0 1 .5-.5h5.793L8.146 5.354a.5.5 0 1 1 .708-.708l3 3a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708-.708L10.293 8.5H4.5A.5.5 0 0 1 4 8"/>
                    </svg>
                    <svg onClick={prevSlide} className='slider-arrow slider-arrow-prev'xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                        <path fillRule="evenodd" d="M4 8a.5.5 0 0 1 .5-.5h5.793L8.146 5.354a.5.5 0 1 1 .708-.708l3 3a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708-.708L10.293 8.5H4.5A.5.5 0 0 1 4 8"/>
                    </svg>
                </div>
                
            </div>
        </div>
    )
}

export default Banner;